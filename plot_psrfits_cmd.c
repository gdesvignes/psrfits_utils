...
usage(void)
{
  fprintf(stderr,"%s","   [-m maskfile] [-verbose] [-fbin fbin] [-tbin tbin] [-start start] [-end end] [-dm dm] [--] infile ...\n");
  fprintf(stderr,"%s","      \n");
  fprintf(stderr,"%s","      Plot PSRFITS search data into a waterfall. Can read a PRESTO mask.\n");
  fprintf(stderr,"%s","      \n");
  fprintf(stderr,"%s","          -m: Mask filename\n");
  fprintf(stderr,"%s","              1 char* value\n");
  fprintf(stderr,"%s","    -verbose: Debugging mode\n");
  fprintf(stderr,"%s","       -fbin: Power-of-2 number of samples to average in frequency\n");
  fprintf(stderr,"%s","              1 int value between 1 and 512\n");
  fprintf(stderr,"%s","              default: `1'\n");
  fprintf(stderr,"%s","       -tbin: Power-of-2 number of samples to average in time\n");
  fprintf(stderr,"%s","              1 int value between 1 and 512\n");
  fprintf(stderr,"%s","              default: `1'\n");
  fprintf(stderr,"%s","      -start: Start time of the data to plot\n");
  fprintf(stderr,"%s","              1 float value between 0.0 and 10000.0\n");
  fprintf(stderr,"%s","              default: `0.0'\n");
  fprintf(stderr,"%s","        -end: End time of the data to plot\n");
  fprintf(stderr,"%s","              1 float value between 0.0 and 10000.0\n");
  fprintf(stderr,"%s","              default: `1.0'\n");
  fprintf(stderr,"%s","         -dm: DM to dedisperse the data\n");
  fprintf(stderr,"%s","              1 float value between 0.0 and 10000.0\n");
  fprintf(stderr,"%s","              default: `0.0'\n");
...
parseCmdline(int argc, char **argv)
{
  ...
  for(i=1, cmd.argc=1; i<argc; i++) {
    ...
    if( 0==strcmp("-fbin", argv[i]) ) {
      int keep = i;
      cmd.fbinP = 1;
      i = getIntOpt(argc, argv, i, &cmd.fbin, 1);
      cmd.fbinC = i-keep;
      checkIntLower("-fbin", &cmd.fbin, cmd.fbinC, 512);
      checkIntHigher("-fbin", &cmd.fbin, cmd.fbinC, 1);
      continue;
    }

    if( 0==strcmp("-tbin", argv[i]) ) {
      int keep = i;
      cmd.tbinP = 1;
      i = getIntOpt(argc, argv, i, &cmd.tbin, 1);
      cmd.tbinC = i-keep;
      checkIntLower("-tbin", &cmd.tbin, cmd.tbinC, 512);
      checkIntHigher("-tbin", &cmd.tbin, cmd.tbinC, 1);
      continue;
    }
    ...
    if( 0==strcmp("-start", argv[i]) ) {
      int keep = i;
      cmd.startP = 1;
      i = getFloatOpt(argc, argv, i, &cmd.start, 1);
      cmd.startC = i-keep;
      checkFloatLower("-start", &cmd.start, cmd.startC, 10000.0);
      checkFloatHigher("-start", &cmd.start, cmd.startC, 0.0);
      continue;
    }
    ...
    if( 0==strcmp("-end", argv[i]) ) {
      int keep = i;
      cmd.endP = 1;
      i = getFloatOpt(argc, argv, i, &cmd.end, 1);
      cmd.endC = i-keep;
      checkFloatLower("-end", &cmd.end, cmd.endC, 10000.0);
      checkFloatHigher("-end", &cmd.end, cmd.endC, 0.0);
      continue;
    }
    ...
    if( 0==strcmp("-dm", argv[i]) ) {
      ...
      cmd.dmC = i-keep;
      checkFloatLower("-dm", &cmd.dm, cmd.dmC, 10000.0);
      checkFloatHigher("-dm", &cmd.dm, cmd.dmC, 0.0);
      continue;
    }
